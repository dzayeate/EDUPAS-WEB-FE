<template>
  ...
  >
    ...
    <div class="flex items-center gap-10 w-full z-20 bg-white">
      <div class="flex justify-between gap-10 items-center w-full md:w-auto">
        <div>
          <a
            v-if="['/'].includes($route.path)"
            @click="navigateTo('home')"
      ...
      <div v-if="!userDetail" class="ml-auto hidden md:flex gap-4">
        <a href="/login">
          <span class="font-medium text-colorText hover:opacity-[0.8]"
            >Login</span
          >
        </a>
        <span>|</span>
        <a href="/register">
          <span class="font-medium text-customBlue hover:opacity-[0.8]"
            >Register</span
          >
        </a>
      </div>
      <div v-else class="ml-auto hidden md:flex gap-4">
        <button
          ...
          class="font-normal text-dark hover:opacity-[0.8] focus:outline-none rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center"
          type="button"
        >
          {{ userDetail.email }} <span class="mx-2">|</span>
...
import { GET_USER_DETAIL } from "@/store/user.module";
...
export default {
  ...
  async mounted() {
    ...
    if (userId) {
      ...
      await this.$store.dispatch(GET_USER_DETAIL, params);
    }
  },
  beforeUnmount() {
